...
flowchart LR
    ...
    VS --> VGB["verifier-grounded-benchmark 0.7.0"]
...
当前 pin 为 `verifier-grounded-benchmark==0.7.0`，wheel SHA256 为
`2cae374f7d5f3b5a2b4724a4360ebb9b1ccbb8eb1040baca590984b84caa8711`。该 release 的正式 track 使用
...
- ArtifactReader 和现有 OpenClaw dashboard 都从配置的 `state/benchmark-runs` 递归发现分类 Run，并按唯一
  `run-id` 查询；
- ArtifactReader 将 `legacy-workspace-archives` 视为保留维护树，从父目录递归扫描或直接将该目录及其
  后代配置为扫描根时均不发现 Run；
